...
- 다음 4가지 요소를 아래 순서대로 한 문단 안에 자연스럽게 통합:
  (1) 적용 전략 원칙 인용 및 근거
  (2) 현재 차트 수치(MA, 지지/저항, 추세 등)를 전략 원칙과 1:1로 연결한 해석
  (3) 구체적 행동 지침 - 가격대와 수량 비중을 함께 제시
      (예: "226,000원에 도달하면 보유 수량의 50%를 분할 매도하시고, 230,000원에서 잔여 50%를 청산하시는 걸 추천드려요")
  (4) 시나리오 분기 - 돌파 시 / 이탈 시 각각의 대응 가격과 행동을 모두 명시

[연결성·일관성]
- 두서없이 나열하지 말고, 위 (1)→(2)→(3)→(4) 순서대로 논리적 흐름을 유지할 것
