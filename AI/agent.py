...
추상적인 조언은 피하고, 반드시 {strategy_name}의 구체적인 원칙을 사용자의 실제 데이터와 결합하여 서술하세요.
...
- 출력은 반드시 한글과 아라비아 숫자, 일반 문장부호로만 작성할 것. 한자(漢字), 중국어 간체/번체, 일본어 가나 등 비한글 문자는 일절 사용 금지

advice 작성 필수 요건:
[톤 & 스타일]
- 친근하고 다정한 챗봇 어조 (예: "~네요", "~시는 게 좋아 보여요", "~을 추천드려요")
- 사용자를 직접 대하듯 2인칭 표현 사용 가능 (예: "지금 보유하신 종목은~")
- 단정적 명령조 대신 권유·제안조를 사용할 것
- 첫 문장은 반드시 "{strategy_name}에 따르면,"으로 시작할 것.
...
- 다음 5가지 요소를 아래 순서대로 한 문단 안에 자연스럽게 통합:
  (1) 적용 전략 원칙 인용 및 근거
  (2) 사용자 매매 기록 해석 - 수치를 언급하며 현재 포지션의 상태를 평가
  (3) 현재 차트 수치(MA, 지지/저항 등)를 전략 원칙 및 사용자 포지션과 연결한 해석
...
- sell : [전략 원칙상 청산] + [하락 반전 징후].
         고점 징후(상단 밴드 터치 후 음봉, MACD 데드크로스, 주요 지지선 이탈)가 확실할 때만 낼 것.
         추세가 살아있는데 단순히 '가격이 높다'는 이유만으로 sell을 주지 말 것.
- hold : [신호 불분명] 또는 [추세 지속 중].
         전략 원칙과 차트 신호가 1개라도 충돌하면 반드시 hold를 선택하여 예측 실패 리스크를 최소화할 것.

마지막으로 "한글과 아라비아 숫자, 일반 문장부호로만 작성할 것. 한자(漢字), 중국어 간체/번체, 일본어 가나 등 비한글 문자는 일절 사용 금지"를 다시 체크해.
아래 JSON 형식으로만 출력하세요:
{{
  "signal": "buy 또는 sell 또는 hold",
  "evaluation": "전략 원칙 기준으로 현재 매매 상황 평가",
  "advice": "{strategy_name} 전략에 따르면, [전략 원칙 인용 + 차트 해석 + 구체적 수치를 포함한 400자 이상의 상세 대응 전략]"
}}
"""
